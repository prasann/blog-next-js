  ...
  return (
    <div className="mb-6">
      <div className="flex items-center gap-3 md:gap-6 mb-3">
        <span className="text-4xl md:text-6xl animate-wave inline-block origin-[70%_70%]">
          👋🏽
        </span>
        <h1 className="text-3xl md:text-6xl font-bold gradient-text drop-shadow-[0_0_20px_rgba(59,130,246,0.4)]">
          Hi, I'm Prasanna !!
        </h1>
      </div>
    </div>
...
const IconLink = ({ navigateLink, icon, linkText }: IconLinkProps) => {
  ...
  return (
    ...
    >
      <FontAwesomeIcon
        icon={icon}
        className="text-2xl md:text-4xl text-theme-accent group-hover:text-theme-accent-light transition-colors"
      />
  ...
  return (
    <div className="space-y-4 text-justify text-theme-text-primary text-base md:text-xl">
      <p className="leading-normal">
        I'm a full-stack developer from the bustling city of Bengaluru,
        India—where navigating traffic is almost as challenging as debugging
        code 🤯 But yes, it's all about finding the right shortcuts, whether on
        the road or in the code! 🚗 I've led large teams to build
        high-performance enterprise platforms. When I'm not doing that, you'll
        find me enjoying coffee and coding ☕
      </p>
      <p className="leading-normal">
        I hold a master's degree in software systems 🎓, which is just a fancy
        way of saying I spent a lot of time in front of a computer screen.
        Currently, I'm working with the amazing folks at Microsoft, but I used
        to be a part of the Thoughtworks family. I'm a certified AWS solution
        architect and I have a few Azure certifications as well. Basically, if
        it's in the cloud, I can craft solutions all around 😎
      </p>
      <p className="leading-normal">
        I have hands-on experience with web technologies, which is a polite way
        of saying I've broken things on the internet a few times 💥. Since
        you're here, why not check out my blogs for some random musings ✍️ or
        visit my talks page to hear more about my adventures in tech 🏃‍♂️
      </p>
      <div className="mt-5 flex flex-wrap justify-center gap-6">
        <IconLink linkText="Blog" navigateLink="/blog" icon={faBlog} />
        <IconLink linkText="Talks" navigateLink="/talks" icon={faMicrophone} />
      </div>
    </div>
  );
};
const Picture = () => {
  return (
    <div className="relative group">
      ...
      <div className="absolute inset-0 w-40 md:w-52 lg:w-60 h-44 md:h-60 lg:h-68 bg-linear-to-br from-blue-500/50 via-blue-400/40 to-blue-600/50 rounded-[50%] blur-3xl -z-10 group-hover:blur-2xl group-hover:scale-110 transition-all duration-500"></div>
