...
const CadenceStrip = ({ years, yearCounts, visibleYears, activeYear, onSelectYear }: CadenceStripProps) => {
          ...
          return (
            <button
              ...
              className={`group flex flex-shrink-0 flex-col items-center gap-1 rounded-md px-1.5 py-1 transition-all duration-200 ${
                isDimmed ? "cursor-not-allowed opacity-30" : "cursor-pointer hover:bg-theme-glass-medium"
              }`}
            >
              <span
                className={`block w-2 rounded-full transition-all duration-200 md:w-2.5 ${
                  isActive ? "bg-theme-cyan ring-2 ring-theme-cyan/40" : "bg-theme-accent"
                }`}
                style={{ height: `${8 + intensity * 20}px`, opacity: isDimmed ? 0.4 : intensity }}
              />
              <span
                className={`text-[10px] md:text-xs ${
                  isActive ? "font-semibold text-theme-cyan-light" : "text-theme-text-muted"
                }`}
              >
                {year}
              </span>
